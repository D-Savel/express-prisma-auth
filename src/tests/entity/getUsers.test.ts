...
export default function getUsersTest(keys: string[]) {
  describe('**** Users CRUD TEST ****', function () {
    ...
    describe("GET USERS", function () {
      ...
      it(`it should all user object in array data.users response containing ${(keys.length)} keys: (${[...keys]})`, async function () {
        for (const user of response.body.data.users) {
